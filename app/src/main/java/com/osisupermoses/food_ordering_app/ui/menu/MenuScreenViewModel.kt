...
class MenuScreenViewModel @Inject constructor(
    ...
    private fun getMenuScreenItems() {
        viewModelScope.launch {
            ...
            getFirestoreUsers.await().onEach { response ->
                when (response) {
                    is Resource.Success -> {
                        ...
                        isAdmin = currentUser?.isSellerAccount == "Yes"
            ...
            when(val result = getRestaurants.await()) {
                is Resource.Success -> {
                    state.value = MenuState(isLoading = false)
                    restaurantList = result.data!!
                    Log.i(TAG, "RESTAURANTS: ${result.data}")
                }
                is Resource.Error -> {
                    state.value = MenuState(
                        isLoading = false,
                    ...
                    Log.i(TAG, "RESTAURANTS: ${result.message!!}")
            ...
            // GET LIST OF FOODS
            when(val result = getFoods.await()) {
                is Resource.Success -> {
                    state.value = MenuState(isLoading = false)
                    foods = result.data!!
                    Log.i(TAG, "FOODLIST: ${state.value}")
                }
                is Resource.Error -> {
                    state.value = MenuState(
                        isLoading = false,
                    ...
                    Log.i(TAG, "FOODS: ${result.message!!}")
